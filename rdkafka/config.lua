...
function KafkaConfig.create(original_config)
    ...
    if original_config and original_config.kafka_conf_ then
        ...
        config:set_consume_cb(original_config.cb_.consume_cb_)
...
--[[
    Set consumer callback in provided conf object.
]]--

function KafkaConfig:set_consume_cb(callback)
    assert(self.kafka_conf_ ~= nil)

    if callback then
        self.cb_.consume_cb_ = callback
        librdkafka.rd_kafka_conf_set_consume_cb(self.kafka_conf_,
            function(rkmessage, opaque)
                local errstr = nil
                if rkmessage.err ~= librdkafka.RD_KAFKA_RESP_ERR_NO_ERROR then
                    errstr = ffi.string(librdkafka.rd_kafka_err2str(err))
                end
                callback(ffi.string(rkmessage.payload, tonumber(rkmessage.len)), errstr)
            end)
    end
end
